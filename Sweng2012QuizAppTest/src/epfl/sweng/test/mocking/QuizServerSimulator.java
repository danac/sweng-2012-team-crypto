 ...
 *
 */
public class QuizServerSimulator extends ServerSimulator {
		...
		try {
			if (request.getRequestLine().getUri().endsWith("125")) {				
				response.setEntity(new StringEntity("{\"id\": 125,\"title\": \"The hardest quiz ever\","
						...
						+ "]}]}"));
			} else if (request.getRequestLine().getUri().endsWith("/submission")) {
				response.setEntity(new StringEntity("{\"score\": 13.58}"));
			}
